...
import { generateThemeProperties, formatAndCleanPluginConfig, plugin } from "./functions/index.ts";
 ...
 */
const extractThemeVariablesFromOptions = ({
  pluginOptions,
  themeName,
}: {
  pluginOptions: PluginOptions;
  themeName: string;
}): { name: string; vars: ThemeColors } | null => {
...
const getThemeNameConfig = (theme: string): { name: string; flag: Flags } | null => {
  ...
  if (name && Object.values(Flags).includes(flag as Flags) && theme.includes(flag)) {
...
const createSemanticThemePlugin = () => {
  return plugin.withOptions(function (options: PluginOptions = {}) {
    return function ({ addBase }) {
      const { root, name, ...pluginOptions } = formatAndCleanPluginConfig(options);

      const nameCnf = getThemeNameConfig(name);
      if (!nameCnf) throw new Error("Invalid theme name configuration");
      const customTheme = extractThemeVariablesFromOptions({
        themeName: nameCnf.name,
        pluginOptions,
      });
