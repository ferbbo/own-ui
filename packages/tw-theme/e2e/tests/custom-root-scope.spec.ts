...
test.describe('Custom Root Scope - Plugin Configuration', () => {
  ...
  test('debe aplicar variables CSS solo en elementos con la clase .app-container', async ({ page }) => {
  ...
  test('NO debe aplicar variables CSS en elementos fuera del scope', async ({ page }) => {
    ...
    const primaryVarOutside = await page.evaluate(() => {
      ...
      return outsideEl ? getComputedStyle(outsideEl).getPropertyValue('--color-primary').trim() : null;
    });
  ...
  test('debe generar CSS variables para todos los colores semánticos dentro del scope', async ({ page }) => {
  ...
  test('debe generar CSS variables con variantes (-content, -focus) dentro del scope', async ({ page }) => {
  ...
  test('clases utilitarias de color deben funcionar en elementos anidados profundamente', async ({ page }) => {
  ...
  test('selector :where(.app-container) debe tener menor especificidad que data-theme', async ({ page }) => {
    ...
    const primaryInDark = await page.evaluate(() => {
      ...
      return darkContainer ? getComputedStyle(darkContainer).getPropertyValue('--color-primary').trim() : null;
    });

    const primaryInLight = await page.evaluate(() => {
      const lightContainer = document.querySelector('.app-container:not([data-theme])');
      return lightContainer ? getComputedStyle(lightContainer).getPropertyValue('--color-primary').trim() : null;
    });
...
test.describe('Custom Root Scope - Edge Cases', () => {
  ...
  test('elementos que dinámicamente obtienen la clase .app-container deben heredar los estilos', async ({ page }) => {
  ...
  test('quitar la clase .app-container debe remover el acceso a las variables CSS', async ({ page }) => {
    // Primero verificar que funciona
    const beforeRemoval = await page.evaluate(() => {
      const container = document.querySelector('.app-container');
      return container ? getComputedStyle(container).getPropertyValue('--color-primary').trim() : null;
    });
    ...
    const afterRemoval = await page.evaluate(() => {
      ...
      return exContainer ? getComputedStyle(exContainer).getPropertyValue('--color-primary').trim() : null;
    });
